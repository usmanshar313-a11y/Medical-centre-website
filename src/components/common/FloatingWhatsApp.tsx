...
export const FloatingWhatsApp: React.FC<FloatingActionsProps> = ({ onOpenBooking }) => {
  ...
  return (
    <>
      {/* Floating Fixed Booking Pill Button (Positioned Directly Above WhatsApp) */}
      <button
        onClick={handleBookingClick}
        className="fixed bottom-[84px] right-6 z-50 group flex items-center gap-2 bg-[#C8102E] hover:bg-[#a00d24] text-white px-4 py-3 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 text-xs sm:text-sm font-bold cursor-pointer border border-white/20"
      ...
      {/* Floating Circular WhatsApp Icon */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-50 group flex items-center justify-center bg-[#25D366] hover:bg-[#20ba5a] text-white p-3.5 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 border border-white/20"
      ...
      >
        ...
        <svg
          className="w-6 h-6 fill-current text-white shrink-0 ml-1"
          viewBox="0 0 24 24"
        ...
        <span className="hidden sm:inline font-bold text-xs tracking-wide ml-1.5">
          WhatsApp Us
        </span>
      </a>
    </>
